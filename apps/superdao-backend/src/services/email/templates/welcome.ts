...
export const generateWelcomeTemplate = (
	{ daoName, daoDescription, daoLink, daoAvatar, hasDaoAvatar }: WelcomeEmailProps & { daoLink: string },
	noTransform?: boolean
) => {
	const mjml = `<mjml>
  <mj-head>
    ...
    <mj-section padding-top="0" background-color="white">
      <mj-column>
        ...
        <mj-text font-size="30px" font-weight="700" line-height="42px">
          You got an NFT from ${daoName} ✨
        </mj-text>
        <mj-spacer height="28px" />
        <mj-text font-weight="400" font-size="19px" line-height="28px">
          Log in to Superdao with your wallet %recipient.walletAddress% to see other members, NFTs, and news of ${daoName}
        </mj-text>
        <mj-spacer height="28px" />
