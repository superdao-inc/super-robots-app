...
export type BuyNftSuccessEmailProps = {
	daoName: string;
	daoSlug: string;
	daoAvatar: string;
...
export const generateBuyNftSuccessTemplate = (
	{
		daoName,
		daoAvatar,
		hasDaoAvatar,
		...
		tierName,
		daoLink
	}: BuyNftSuccessEmailProps & { daoLink: string },
	noTransform?: boolean
) => {
	const mjml = `<mjml>
  <mj-head>
  ...
  <mj-body background-color="#fcf8f4">
    <mj-section padding-top="0" background-color="white">
      <mj-column>
        ...
        You bought an NFT from ${daoName}! 🎉
        </mj-text>
        <mj-spacer height="28px" />
        <mj-text font-weight="400" font-size="19px" line-height="28px">
          The ${daoName} NFT is now in your wallet ${shrinkWallet(walletAddress)}.
        </mj-text>
        <mj-spacer height="8px" />
        <mj-text font-weight="400" font-size="19px" line-height="28px">
          Log in to Superdao with this wallet to see other members, chats, and news of ${daoName}
        </mj-text>
        <mj-spacer height="28px" />
        ...
        <mj-hero mode="fluid-height" height="300px" background-url="https://app.superdao.co/assets/email-confetti.png" padding-top="5px" padding-bottom="14px">
          <mj-section css-class="section-width" background-color="#252B36" border-radius="12px" padding="11px">
            <mj-image src="${tierImg}" border-radius="6px" width="190px" height="190px" />
            <mj-spacer height="11px" />
            <mj-text font-weight="700" font-size="16px" line-height="19px" color="white">${tierName}</mj-text>
