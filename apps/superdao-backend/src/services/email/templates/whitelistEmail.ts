...
export type WhitelistEmailProps = {
	daoName: string;
	daoSlug: string;
};

export const generateWhitelistEmailTemplate = (
	{ daoName, daoLink }: WhitelistEmailProps & { daoLink: string },
	noTransform?: boolean
) => {
	const mjml = `<mjml>
  <mj-head>
    ...
    <mj-section padding-top="0" background-color="white">
      <mj-column>
        ...
        <mj-text font-size="30px" font-weight="700" line-height="42px">
          You got on the ${daoName} whitelist! 🔥
        </mj-text>
        <mj-spacer height="28px" />
        <mj-text font-weight="400" font-size="19px" line-height="28px">
          Now you can get a membership NFT and join the ${daoName} community!
        </mj-text>
        <mj-spacer height="8px" />
        <mj-text font-weight="400" font-size="19px" line-height="28px">
          To mint your NFT, log in to Superdao with wallet %recipient.walletAddress%
        </mj-text>
        <mj-spacer height="28px" />
